 ...
 *
 * Identical feature set to the default theme, minus theme-toggle
 * (nozilla is always light / paper-and-ink).
 *
 * @package   PureWiki
 * @license   GNU AGPLv3
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Sticky Header — border appears on scroll instead of glass effect
    ...
    document.querySelectorAll('.pw-code-copy').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            try {
                ...
                setTimeout(() => { btn.classList.remove('success'); icon.setAttribute('icon', originalIcon); }, 2000);
    ...
    if (toTopBtn) {
        window.addEventListener('scroll', () => toTopBtn.classList.toggle('pw-visible', window.scrollY > 300));
    ...
    if (searchToggle && searchDropdown && searchInput && searchResults) {
        ...
        async function performSearch(query) {
            ...
            } catch (e) { searchResults.innerHTML = '<div class="pw-search-empty">Suche fehlgeschlagen.</div>'; }
        }
